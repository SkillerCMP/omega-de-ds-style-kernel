...
typedef struct NOR_ROM_READ_CONTEXT_
{
	FIL *file;
} NOR_ROM_READ_CONTEXT;

typedef struct NOR_ROM_WRITE_CONTEXT_
{
	u8 *block;
	u32 block_offset;
	u32 block_size;
} NOR_ROM_WRITE_CONTEXT;

static u32 ReadNORRomByte(u32 offset, u8 *value, void *context)
{
	NOR_ROM_READ_CONTEXT *rom = (NOR_ROM_READ_CONTEXT*)context;
	UINT bytes_read = 0;

	if (rom == NULL || rom->file == NULL || value == NULL)
		return 0;
	if (f_lseek(rom->file, offset) != FR_OK)
		return 0;
	if (f_read(rom->file, value, 1, &bytes_read) != FR_OK || bytes_read != 1)
		return 0;
	return 1;
}

static u32 WriteNORRomByte(u32 offset, u8 value, void *context)
{
	NOR_ROM_WRITE_CONTEXT *rom = (NOR_ROM_WRITE_CONTEXT*)context;

	if (rom == NULL || rom->block == NULL)
		return 0;
	if (offset >= rom->block_offset &&
		offset < rom->block_offset + rom->block_size)
	{
		rom->block[offset - rom->block_offset] = value;
	}
	return 1;
}
//---------------------------------------------------------------
...
u32 Loadfile2NOR(TCHAR *filename, u32 NORaddress,u16 have_patch,u8 SAVEMODE)
{
	...
	if(norid == 0x223D)//S98
	{
		...
		if (have_patch && gl_cheat_on == 1)
		{
			NOR_ROM_READ_CONTEXT rom_read_context;
			rom_read_context.file = &gfile;
			/* ROMIF: validates against the original file before block patching. */
			EvaluateEnhancedCheatGroups(ReadNORRomByte, &rom_read_context, filesize);
		}
		f_lseek(&gfile, 0xa0);		
		...
		*((vu16 *)(FlashBase_S98+0x000*2)) = 0x00 ;			
		...
		{		
			...
			if (have_patch && gl_cheat_on == 1)
			{
				...
				rom_write_context.block = pReadCache;
				rom_write_context.block_offset = blocknum;
				rom_write_context.block_size = 0x20000;
				/* Apply selected ROM: bytes after the kernel's own block edits. */
				ApplyActiveRomPatches(WriteNORRomByte, &rom_write_context, filesize);
			}
